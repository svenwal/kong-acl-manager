...
docker buildx build --push --platform linux/amd64,linux/arm64 --no-cache -t svenwal/kong-acl-manager:$tag .
echo "--- 2. Building current version as latest"
docker buildx build --push --platform linux/amd64,linux/arm64 -t svenwal/kong-acl-manager:latest .
